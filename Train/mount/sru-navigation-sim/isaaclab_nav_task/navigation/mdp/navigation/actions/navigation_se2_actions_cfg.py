...
class PerceptiveNavigationSE2ActionCfg(ActionTermCfg):
    ...
    Set to ``None`` for legs-only quadrupeds (e.g. Go2) whose locomotion policy
    only outputs joint position targets. The position-action branch then handles
    all 12 leg joints, and the velocity-action branch is fully bypassed."""
