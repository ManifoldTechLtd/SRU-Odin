...
class PerceptiveNavigationSE2Action(ActionTerm):
    ...
    def __init__(self, cfg: PerceptiveNavigationSE2ActionCfg, env: ManagerBasedRLEnv):
        ...
        # Velocity branch is optional (None for legs-only robots like Go2).
        if self.cfg.low_level_velocity_action is not None:
            self.low_level_velocity_action_term: ActionTerm | None = self.cfg.low_level_velocity_action.class_type(cfg.low_level_velocity_action, env)
        else:
            self.low_level_velocity_action_term = None
    ...
    def apply_actions(self):
        ...
        if self._counter % self.cfg.low_level_decimation == 0:
            ...
            pos_dim = self.low_level_position_action_term.action_dim
            self._low_level_position_actions[:] = actions_phase[:, :pos_dim]
            self.low_level_position_action_term.process_actions(self._low_level_position_actions)
            if self.low_level_velocity_action_term is not None:
                self._low_level_velocity_actions[:] = actions_phase[:, pos_dim:]
    ...
    def _init_buffers(self):
        ...
        vel_dim = self.low_level_velocity_action_term.action_dim if self.low_level_velocity_action_term is not None else 0
        self._low_level_velocity_actions = torch.zeros(self.num_envs, vel_dim, device=self.device)
